...
This script specifically, is used to process mean seasonal
    precip for cool and warm seasons and annual
...
# %% this section was just to investigate that thins seemed reasonable
# uncomment if want to look at seasonal precip totals for different
# calendar years
####################################
...
an_mean_wgs84 = GF.reproject_dataarray(an_mean_spat)
cool_mean_wgs84 = GF.reproject_dataarray(cool_mean_spat)
warm_mean_wgs84 = GF.reproject_dataarray(warm_mean_spat)

an_mean_wgs84.rio.to_raster(
    Path(DIR_TIFFS, f"CONUS404_MeanAnnualPrec_{suffix}.tif"),
    driver="GTiff"
    )
cool_mean_wgs84.rio.to_raster(
    Path(DIR_TIFFS, f"CONUS404_MeanCoolSeasonPrec_{suffix}.tif"),
    driver="GTiff"
    )
warm_mean_wgs84.rio.to_raster(
    Path(DIR_TIFFS, f"CONUS404_MeanWarmSeasonPrec_{suffix}.tif"),
    driver="GTiff"
    )
...
# %% Calculate mean total seasonal precipitation for each season for each
# grid cell
################################################
# NOTE: persist will make this computationally/data-transfer
# "expensive". Values will be stored locally.
...
season_plot = "annual"  # cool, warm, or annual
das = {
    "cool": cool_total_spat,
    "warm": warm_total_spat,
    "annual": an_total_spat
}
da = das[season_plot]
# da = ds_prec_cool.groupby("time.year").total(dim="time").sel(year=1999)
# da = cool_total_spat
# da = warm_total_spat
...
# %% save seasonal totals
################################################

an_total_wgs84 = GF.reproject_dataarray(an_total_spat)
cool_total_wgs84 = GF.reproject_dataarray(cool_total_spat)
warm_total_wgs84 = GF.reproject_dataarray(warm_total_spat)

an_total_wgs84.rio.to_raster(
    Path(DIR_TIFFS, f"CONUS404_totalAnnualPrec_{suffix}.tif"), driver="GTiff"
    )
cool_total_wgs84.rio.to_raster(
    Path(DIR_TIFFS, f"CONUS404_totalCoolSeasonPrec_{suffix}.tif"), driver="GTiff"
    )
warm_total_wgs84.rio.to_raster(
    Path(DIR_TIFFS, f"CONUS404_totalWarmSeasonPrec_{suffix}.tif"), driver="GTiff"
    )
print(f"Saved total and total rasters to {DIR_TIFFS}")


# %% get versions of mean and total output rasters resampled to 
# common grid size and extent as other rasters used in OWRD precip freq. work
##########################################

# resample mean rasters
an_mean_resampled = GF.resample_dataarray_to_match(
    an_mean_wgs84,
    "C:/Projects/OWRD/PrecipFreq/ExplRasters/treecover_average.tif",
    resampling_method='bilinear'
)
cool_mean_resampled = GF.resample_dataarray_to_match(
    cool_mean_wgs84,
    "C:/Projects/OWRD/PrecipFreq/ExplRasters/treecover_average.tif",
    resampling_method='bilinear'
)
warm_mean_resampled = GF.resample_dataarray_to_match(
    warm_mean_wgs84,
    "C:/Projects/OWRD/PrecipFreq/ExplRasters/treecover_average.tif",
    resampling_method='bilinear'
)
...
# save mean rasters
an_mean_resampled.rio.to_raster(
    Path(DIR_TIFFS, f"CONUS404_meanAnnualPrec_Resampled_{suffix}.tif"),
    driver="GTiff"
    )
cool_mean_resampled.rio.to_raster(
    Path(DIR_TIFFS, f"CONUS404_meanCoolSeasonPrec_Resampled_{suffix}.tif"),
    driver="GTiff"
    )
warm_mean_resampled.rio.to_raster(
    Path(DIR_TIFFS, f"CONUS404_meanWarmSeasonPrec_Resampled_{suffix}.tif"),
    driver="GTiff"
    )


# resample total rasters
an_total_resampled = GF.resample_dataarray_to_match(
    an_total_wgs84,
    "C:/Projects/OWRD/PrecipFreq/ExplRasters/treecover_average.tif",
    resampling_method='bilinear'
)
cool_total_resampled = GF.resample_dataarray_to_match(
    cool_total_wgs84,
    "C:/Projects/OWRD/PrecipFreq/ExplRasters/treecover_average.tif",
    resampling_method='bilinear'
)
warm_total_resampled = GF.resample_dataarray_to_match(
    warm_total_wgs84,
    "C:/Projects/OWRD/PrecipFreq/ExplRasters/treecover_average.tif",
    resampling_method='bilinear'
)
...
# save total rasters
an_total_resampled.rio.to_raster(
    Path(DIR_TIFFS, f"CONUS404_totalAnnualPrec_Resampled_{suffix}.tif"),
    driver="GTiff"
    )
cool_total_resampled.rio.to_raster(
    Path(DIR_TIFFS, f"CONUS404_totalCoolSeasonPrec_Resampled_{suffix}.tif"),
    driver="GTiff"
    )
warm_total_resampled.rio.to_raster(
    Path(DIR_TIFFS, f"CONUS404_totalWarmSeasonPrec_Resampled_{suffix}.tif"),
    driver="GTiff"
    )